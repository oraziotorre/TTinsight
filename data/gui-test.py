...
def aggiungi_punto_utente():
    ...
    if len(punteggi) < 20 and not gioco_terminato:  # Limitiamo a 20 iterazioni e gioco attivo
        punteggi.append(1)
        aggiorna_punteggi()
        verifica_vittoria()
...
def aggiorna_punteggi():
    ...
    # Calcoliamo il punteggio dell'utente e dell'avversario
...
root.grid_rowconfigure(0, weight=0)  # Riga per il titolo
root.grid_rowconfigure(1, weight=1)  # Riga per i frame principali

# Creazione del titolo sopra entrambe le colonne
titolo = tk.Label(root, text="TTinsight", font=("Helvetica", 72, "bold"), fg="#333", bg="#ffffff")
titolo.grid(row=0, column=0, columnspan=2, sticky="ew", pady=20)
...
etichetta_punteggi = tk.Label(left_frame, text="0-0", font=("Helvetica", 64, "bold"), fg="#333", bg="#f0f0f0")
etichetta_punteggi.place(relx=0.5, rely=0.3, anchor="center")

# Aggiunta bottoni
bottone_utente = tk.Button(left_frame, text="+", command=aggiungi_punto_utente, font=("Arial", 36, "bold"), bg="#77DD77", fg="white", width=4, height=1, relief="ridge")
bottone_utente.place(relx=0.25, rely=0.7, anchor="center")

bottone_reset = tk.Button(left_frame, text="Reset", command=reset, font=("Arial", 36, "bold"), bg="#FFD700", fg="white", width=8, height=1, relief="ridge")
bottone_reset.place(relx=0.5, rely=0.7, anchor="center")

bottone_avversario = tk.Button(left_frame, text="+", command=aggiungi_punto_avversario, font=("Arial", 36, "bold"), bg="#FF6961", fg="white", width=4, height=1, relief="ridge")
bottone_avversario.place(relx=0.75, rely=0.7, anchor="center")
...
# Aggiunta della parola "hello" al centro del frame di destra
